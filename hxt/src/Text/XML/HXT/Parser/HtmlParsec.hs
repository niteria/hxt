...
import Text.XML.HXT.Parser.XmlParsec (misc, parseXmlText, xMLDecl')
...
parseHtmlFromString parser loc =
  ...
  runParser parser (withNormNewline ()) loc
    ...
    option
      ...
      (try doctypedecl <|>
       (do pos <- getPosition
           ...
           return $
             [mkError' c_warn (show pos ++ " HTML DOCTYPE declaration ignored")]))
...
htmlContent = mergeTextNodes <$> htmlContent'

htmlContent' :: SimpleXParser XmlTrees
htmlContent' =
  option
    []
    (do context <- hContent (id, [])
        pos <- getPosition
        return $ closeTags pos context)
  where
    closeTags _pos (body, []) = body []
    closeTags pos' (body, ((tn, al, body1):restOpen)) =
      closeTags
        pos'
        (addHtmlWarn
           (show pos' ++ ": no closing tag found for \"<" ++ tn ++ " ...>\"") .
...
hElement context =
  ...
  (do pos <- getPosition
      ...
      return
        (addHtmlWarn
           (show pos ++
            " markup char " ++ show c ++ " not allowed in this context") .
      ...
      return
        (addHtmlWarn
           (show pos ++
            ...
            show c ++ " found and ignored, possibly wrong encoding scheme used") $
         context))

hSimpleData :: SimpleXParser XmlTree
hSimpleData = charData'' <|> hReference' <|> hComment <|> hpI <|> hcDSect
  where
    charData'' = do
...
hOpenTagRest ((pos, tn), al) context =
  ...
  (do context1 <-
        checkSymbol
          gt
          ("closing > in tag \"<" ++ tn ++ "...\" expected")
          context
      return
        (let context2 = closePrevTag pos tn context1
          in (if isEmptyHtmlTag tn
                then addHtmlTag tn al id
                else openTag tn al)
               context2))

hAttrList :: SimpleXParser XmlTrees
hAttrList = many (try hAttribute)
  where
    hAttribute = do
      n <- lowerCaseName
      v <- hAttrValue
...
hAttrValue = option [] (eq >> hAttrValue')
...
hAttrValue'' notAllowed = many (hReference' <|> singleCharsT notAllowed)
...
hContent context = option context (hElement context >>= hContent)
  ...
  where
    closeCmt pos c =
      ...
      (return $
       mkError'
         c_warn
         (show pos ++ " no closing comment sequence \"-->\" found"))
...
hpI =
  ...
  (try
   ...
   (do pos <- getPosition
       return $ mkError' c_warn (show pos ++ " illegal PI found")))
...
addHtmlTag tn al body context = e `seq` addHtmlElem e context
  where
    e = mkElement' (mkName tn) al (body [])

addHtmlWarn :: String -> Context -> Context
addHtmlWarn msg = addHtmlElem (mkError' c_warn msg)

addHtmlElem :: XmlTree -> Context -> Context
addHtmlElem elem' (body, openTags) = (body . (elem' :), openTags)

openTag :: String -> XmlTrees -> Context -> Context
openTag tn al (body, openTags) = (id, (tn, al, body) : openTags)

closeTag :: SourcePos -> String -> Context -> Context
closeTag pos n context
  | n `elem` (map (\(n1, _, _) -> n1) $ snd context) = closeTag' n context
  | otherwise =
    addHtmlWarn (show pos ++ " no opening tag found for </" ++ n ++ ">") .
    addHtmlTag n [] id $
    context
  where
    closeTag' n' (body', (n1, al1, body1):restOpen) = close context1
      where
        context1 = addHtmlTag n1 al1 body' (body1, restOpen)
        close
          | n' == n1 = id
          | n1 `isInnerHtmlTagOf` n' = closeTag pos n'
          | otherwise =
            addHtmlWarn
              (show pos ++ " no closing tag found for \"<" ++ n1 ++ " ...>\"") .
            closeTag' n'
    closeTag' _ _ = error "illegal argument for closeTag'"

closePrevTag :: SourcePos -> String -> Context -> Context
closePrevTag _pos _n context@(_body, []) = context
  ...
  | n `closesHtmlTag` n1 =
    closePrevTag
      ...
      (addHtmlWarn
         (show pos ++
          " tag \"<" ++
          n1 ++ " ...>\" implicitly closed by opening tag \"<" ++ n ++ " ...>\"") .
...
isEmptyHtmlTag n = n `elem` emptyHtmlTags
...
closesHtmlTag t t2 = fromMaybe False . fmap ($ t) . M.lookup t2 $ closedByTable
